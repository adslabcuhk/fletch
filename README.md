...
- This repo is the source code for our paper `Fletch: File-System Metadata Caching in Programmable Switches`, including how to:
    - deploy Fletch
    - run experiments
...
- 2. [Environment Preparation](#2-environment-preparation)
    - 2.1 [Boost Library](#21-boost-library)
    - 2.2 [RocksDB](#22-rocksdb)
    - 2.3 [Other Dependencies](#23-other-dependencies)
    - 2.4 [Run HDFS (for NoCache and Fletch only)](#24-run-hdfs-for-nocache-and-fletch-only)
- 3. [Compilation](#3-compilation)
    - 3.1 [Common Library](#31-common-library)
    - 3.2 [Client](#32-client)
    - 3.3 [Server, Controller, and SwitchOS](#33-server-controller-and-switchos)
...
- 4. [Start Testbed](#4-start-testbed)
    ...
    - 4.2 [Run Testbed with Scripts (only for 2 Physical Servers)](#42-run-testbed-with-scripts-only-for-2-physical-servers)
    - 4.3 [Run Testbed without Scripts (for 1 Physical Server)](#43-run-testbed-without-scripts-for-1-physical-server)
...
- 6. [Load Dataset to Server](#6-load-dataset-to-server)
    - 6.1 [For Two Physical Servers (for NoCache and Fletch only)](#61-for-two-physical-servers-for-nocache-and-fletch-only)
    - 6.2 [For One Physical Server (for NoCache and Fletch only)](#62-for-one-physical-server-for-nocache-and-fletch-only)
    - 6.3 [Load Dataset to RocksDB (for CCache and Fletch+ only)](#63-load-dataset-to-rocksdb-for-ccache-and-fletch-only)
    - 6.4 [Prepare RocksDB Instance for Recovery](#64-prepare-rocksdb-instance-for-recovery)
- 7. [Experiments](#7-experiments)
    - 7.1 [Run Experiments Automatically](#71-run-experiments-automatically)
    - 7.2 [Experiment Scripts (Replace the Script in 7.1 for Each Experiment)](#72-experiment-scripts-replace-the-script-in-71-for-each-experiment)
    - 7.3 [Get Numbers Presented in the Paper](#73-get-numbers-presented-in-the-paper)
- 8. [Handle Corner Cases During Experiments](#8-handle-corner-cases-during-experiments)


## 1. Important Folders and Binaries

- home directory {$PROJ_HOME}
    - server and client: `/home/jz/Fletch`
    - switch: `/home/cyh/jz/Fletch`
...
- Install RocksDB v6.22.1 in servers and switchos (for hash-token and path-token map recovery) if not
    - Under project directory `{$PROJ_HOME}/netfetch-private`
```bash
...
- in `{$PROJ_HOME}/netfetch-private/common`, run `make`

### 3.2 Client

- in `$CLIENT_HOME`, run `make`

### 3.3 Server, Controller and SwitchOS
...
### 6.2 For One Physical Server (for NoCache and Fletch only)
...
- before loading dataset into RocksDB in each server
...
- in each server, in `$PROJ_HOME/netfetch-private/cscaching/`
...
### 6.4 Prepare RocksDB Instance for Recovery

- create folders for DB instancce in switchos
    - path-token map: `mkdir /tmp/path; mkdir /tmp/path/historical.db; mkdir /tmp/path/active.db`
    - hash-token map: `mkdir /tmp/hash; mkdir /tmp/hash/historical.db; mkdir /tmp/hash/active.db`
- start switchos after we have the above folders
...
3. load dataset (optional, if alreadly loaded, no need to do this again)
    - in `$CLIENT_HOME`, run `python3 exp_thpt.py --config-name load-hdfs`
    - set `needload` as `true` and `rotation` as `false` for NoCache and Fletch
...
- exp1 (recirculation overhead analysis)
    - use one physical machine
    - run rotations: `python3 exp_recir_count.py --config-name exp1-recir`
    - obtain recirculation count:
        - `cd $CLIENT_HOME/recir-exp/`
        - `python3 cal_recir_counts.py`
    - measure recirculation overhead
        - modify the `method_name` in `$CLIENT_HOME/start_server.py` as `netfetch`
        - start the testbed: `python3 start_server.py`
        - warmup cache for recirculatin test: `cd $CLIENT_HOME/recir-exp; sudo python3 recir.py`
        - `cd $CLIENT_HOME/recir-exp/dpdk_bench/`
        - `make clean; make -j8`
        - `sudo ./build/bench -l 0-K -n 4 -- --rate R --count 32000000 --recir C --depth D --tx-lcores M --rx-lcores N`
            - send/recv pool: M + N = K
            - R: sending rate, divided by all tx-lcores
            - C: recirculation count - 1
            - D: path level + 1
...
- exp10 (system recovery)
    - server recovery: `./server 0 1`
    - controller recovery: `./switchos 1`
    - switch recovery
        - export active paths in `switchos` and store the paths in `$CLIENT_HOME/active_path.log`
        - re-populate hash-token map and in-switch cache: `python3 warmup_from_log.py`

- expS1 (recirculation overhead under high write ratio)
    - modify the `method_name` in `$CLIENT_HOME/start_server.py` as `recirthpt`
    - start the testbed: `python3 start_server.py`
    - warmup cache for recirculatin test: `cd $CLIENT_HOME/recir-exp; sudo python3 recir.py
    - `cd $CLIENT_HOME/recir-exp/dpdk_bench/`
    - `make clean; make -j8`
