...
// server
void get_index_db_path(std::string& db_path, uint16_t workerid) {
    GET_STRING(db_path, "/tmp/index/worker" << workerid << ".db");
    return;
}

void get_hash_db_path(std::string& db_path, bool is_historical) {
    if (is_historical){
        GET_STRING(db_path, "/tmp/hash/historical.db");
    } else {
        GET_STRING(db_path, "/tmp/hash/active.db");
    }
    return;
}

void get_path_db_path(std::string& db_path, bool is_historical) {
    if (is_historical){
        GET_STRING(db_path, "/tmp/path/historical.db");
    } else {
        GET_STRING(db_path, "/tmp/path/active.db");
    }
    return;
}
...
void get_index_latestmaxseq_path(method_t methodid, std::string& latestmaxseq_path, uint16_t workerid) {
    GET_STRING(latestmaxseq_path, "/tmp/index/worker" << workerid << ".latestmaxseq");
    return;
}

void get_path_latestmaxseq_path(std::string& latestmaxseq_path, bool is_historical) {
    if (is_historical) {
        GET_STRING(latestmaxseq_path, "~/path/historical.latestmaxseq");
    } else {
        GET_STRING(latestmaxseq_path, "~/path/active.latestmaxseq");
    }
    return;
}

void get_hash_latestmaxseq_path(std::string& latestmaxseq_path, bool is_historical) {
    if (is_historical) {
        GET_STRING(latestmaxseq_path, "~/hash/historical.latestmaxseq");
    } else {
        GET_STRING(latestmaxseq_path, "~/hash/active.latestmaxseq");
    }
    return;
}
...
void get_controller_snapshotid_path(method_t methodid, std::string& snapshotid_path, int controller_idx) {
    const char* methodname = get_methodname_byid(methodid);
    std::string controller_idx_string = controller_idx == -1 ? "" : std::to_string(controller_idx);
    GET_STRING(snapshotid_path, "/tmp/" << std::string(methodname) << "/controller" << controller_idx_string << ".snapshotid");
    return;
}

void get_controller_snapshotdata_path(method_t methodid, std::string& snapshotdata_path, int snapshotid, int controller_idx) {
    const char* methodname = get_methodname_byid(methodid);
    std::string controller_idx_string = controller_idx == -1 ? "" : std::to_string(controller_idx);
    GET_STRING(snapshotdata_path, "/tmp/" << std::string(methodname) << "/controller" << controller_idx_string << ".snapshotdata" << snapshotid);
    return;
}
...
void load_max_dir_id(uint64_t& maxseq, std::string maxseq_path) {
    FILE* maxseq_fd = fopen(maxseq_path.c_str(), "rb");
    fread(&maxseq, sizeof(uint64_t), 1, maxseq_fd);
    fclose(maxseq_fd);
    return;
}
...
void store_max_dir_id(uint64_t maxseq, std::string maxseq_path) {
    FILE* maxseq_fd = fopen(maxseq_path.c_str(), "wb+");
    fwrite(&maxseq, sizeof(uint32_t), 1, maxseq_fd);
    fclose(maxseq_fd);
    return;
...
std::string uint64_to_hex(uint64_t value) {
    std::stringstream ss;
    ss << std::hex << std::uppercase << value;  // 转换为16进制，使用大写字母
    return ss.str();
}
uint64_t hex_to_uint64(const std::string& hex_str) {
    if (hex_str.empty()) {
        throw std::invalid_argument("Input hex string is empty");
    }
...
void get_controller_snapshotid_path(method_t methodid, char* snapshotid_path, int len, int controller_idx) {
    const char* methodname = get_methodname_byid(methodid);
    char tmpstr[256];
    memset(tmpstr, '\0', 256);
    std::string controller_idx_string = controller_idx == -1 ? "" : std::to_string(controller_idx);
    sprintf(tmpstr, "/tmp/%s/controller%s.snapshotid", methodname, controller_idx_string.c_str());
...
void get_controller_snapshotdata_path(method_t methodid, char* snapshotdata_path, int len, int snapshotid, int controller_idx) {
    const char* methodname = get_methodname_byid(methodid);
    char tmpstr[256];
    memset(tmpstr, '\0', 256);
    std::string controller_idx_string = controller_idx == -1 ? "" : std::to_string(controller_idx);
    sprintf(tmpstr, "/tmp/%s/controller%s.snapshotdata%d", methodname, controller_idx_string.c_str(), snapshotid);
