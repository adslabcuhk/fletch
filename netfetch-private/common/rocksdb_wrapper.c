...
bool RocksdbWrapper::open_hash_map(bool is_historical) {
    dbtype = DATADB;
    bool is_runtime_existing = false;
    bool is_snapshot_existing = false;

    std::string db_path;
    get_hash_db_path(db_path, is_historical);
    is_runtime_existing = isexist(db_path);
    rocksdb::Status s = rocksdb::DB::Open(rocksdb_options, db_path, &db_ptr);
    if (!s.ok()) {
        printf("Please create directory for %s, %s\n", db_path.c_str(), s.ToString().c_str());
    ...
    std::string latestmaxseq_path;
    get_hash_latestmaxseq_path(latestmaxseq_path, is_historical);
    if (isexist(latestmaxseq_path)) {
        uint64_t tmpmaxseq = 0;
        load_max_dir_id(tmpmaxseq, latestmaxseq_path);
        if (tmpmaxseq > maxseq) {
...
bool RocksdbWrapper::open_path_map(bool is_historical) {
    dbtype = DATADB;
    bool is_runtime_existing = false;
    bool is_snapshot_existing = false;

    std::string db_path;
    get_path_db_path(db_path, is_historical);
    is_runtime_existing = isexist(db_path);
    rocksdb::Status s = rocksdb::DB::Open(rocksdb_options, db_path, &db_ptr);
    if (!s.ok()) {
        printf("Please create directory for %s, %s\n", db_path.c_str(), s.ToString().c_str());
    ...
    std::string latestmaxseq_path;
    get_path_latestmaxseq_path(latestmaxseq_path, is_historical);
    if (isexist(latestmaxseq_path)) {
        uint64_t tmpmaxseq = 0;
        load_max_dir_id(tmpmaxseq, latestmaxseq_path);
        if (tmpmaxseq > maxseq) {
...
bool RocksdbWrapper::get_all_path_tokens(
    ...
    rocksdb::Iterator* it = db_ptr->NewIterator(rocksdb::ReadOptions());
    for (it->SeekToFirst(); it->Valid(); it->Next()) {
        std::string path = it->key().ToString();
        std::string valstr = it->value().ToString();
        if (valstr.size() == 1) {
            uint8_t token = static_cast<uint8_t>(valstr[0]);
            out.emplace_back(std::move(path), token);
        }
    }
...
bool RocksdbWrapper::put_path_token(std::string queried_path, uint8_t token) {
    while (true) {
        if (rwlock.try_lock())
            break;
    }
    ...
    rocksdb::Status s;
    // encode token as a single byte
    std::string valstr(1, static_cast<char>(token));
...
bool RocksdbWrapper::delete_path_token(std::string queried_path) {
    while (true) {
        if (rwlock.try_lock())
            break;
    }
    ...
    rocksdb::Status s = db_ptr->Delete(write_options, queried_path);
    INVARIANT(s.ok());
...
// encode (hash, token) as a 9-byte key:
//   bytes 0-7: 8-byte big-endian hash
//   byte 8:    1-byte token
static std::string encode_hash_token_key(uint64_t hash, uint8_t token) {
    char key[9];
    for (int i = 7; i >= 0; i--) {
        key[i] = static_cast<char>(hash & 0xff);
        hash >>= 8;
    }
    key[8] = static_cast<char>(token);
    return std::string(key, 9);
}

// decode a 9-byte key back into (hash, token)
static void decode_hash_token_key(const std::string& key, uint64_t& hash,
                                  uint8_t& token) {
    hash = 0;
    for (int i = 0; i < 8; i++) {
        hash = (hash << 8) | static_cast<uint8_t>(key[i]);
    }
    token = static_cast<uint8_t>(key[8]);
}

bool RocksdbWrapper::put_hash_token(uint64_t hash, uint8_t token) {
    while (true) {
        if (rwlock.try_lock())
            break;
    }

    std::string keystr = encode_hash_token_key(hash, token);
    std::string valstr;  // empty value

    rocksdb::WriteOptions write_options;
    write_options.sync = SYNC_WRITE;
    write_options.disableWAL = DISABLE_WAL;

    rocksdb::Status s = db_ptr->Put(write_options, keystr, valstr);
    INVARIANT(s.ok());
...
bool RocksdbWrapper::delete_hash_token(uint64_t hash, uint8_t token) {
    while (true) {
        if (rwlock.try_lock())
            break;
    }
    ...
    rocksdb::Status s = db_ptr->Delete(write_options, keystr);
    INVARIANT(s.ok());
...
bool RocksdbWrapper::get_all_hash_tokens(
    ...
    for (it->SeekToFirst(); it->Valid(); it->Next()) {
        ...
        if (key.size() != 9) continue;
        uint64_t hash;
        uint8_t token;
        decode_hash_token_key(key, hash, token);
        out.emplace_back(hash, token);
    }
    bool ok = it->status().ok();
    delete it;
