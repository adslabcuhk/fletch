...
#define DISABLE_AUTO_COMPATION true
#ifdef USE_TOMMYDS_KVS
...
class RocksdbWrapper {
	...
	public:
		...
		bool open_index(uint16_t tmpworkerid);
		bool open_hash_map(bool is_historical);
		bool open_path_map(bool is_historical);
		...
		// bool get_all_paths(std::vector<std::string> &keys);
		// transaction phase
		bool get(netreach_key_t key, val_t &val, uint32_t *seqptr=NULL);
		bool get_path(std::string queried_path, val_t &val, uint32_t *seqptr=NULL);
		bool get_path_with_id(std::string id,std::string queried_path, val_t &val, uint32_t *seqptr=NULL);
		bool get_directory_index(std::string queried_path, uint64_t& idx);
		bool get_dirs_index(std::vector<std::string>& queried_paths, std::vector<uint64_t>& results);
		...
		bool put_path(std::string queried_path, val_t val, uint32_t seq=0, bool checkseq=false);
		bool put_path_with_id(std::string id, std::string queried_path, val_t val, uint32_t seq=0, bool checkseq=false);
		bool rename_file_with_id(std::string id, std::string queried_path, std::string new_queried_path, val_t val, uint32_t seq=0, bool checkseq=false);
		uint64_t put_directory_index(std::string queried_path);
		bool put_dirs_index(std::vector<std::string>& queried_paths);
		bool remove(netreach_key_t key, uint32_t seq=0, bool checkseq=false);
		bool remove_path(std::string queried_path, uint32_t seq=0, bool checkseq=false);
		bool remove_path_with_id(std::string id, std::string queried_path, uint32_t seq=0, bool checkseq=false);
		bool remove_directory_index(std::string queried_path);
		bool remove_dirs_index(std::vector<std::string>& queried_paths);
		...
		size_t scan_path_with_id(std::string id, std::vector<std::pair<std::string, val_t>> &results);
		size_t load_entire_db(std::unordered_map<std::string, uint64_t>& dir_id_map);
		bool dir_has_son(std::string id);

		// path-token map
		bool put_path_token(std::string queried_path, uint8_t token);
		bool get_all_path_tokens(std::vector<std::pair<std::string, uint8_t>>& out);
		bool delete_path_token(std::string queried_path);

		// hash-token map
		bool put_hash_token(uint64_t hash, uint8_t token);
		bool delete_hash_token(uint64_t hash, uint8_t token);
		bool get_all_hash_tokens(std::vector<std::pair<uint64_t, uint8_t>>& out);

		bool flush_and_compact();
		
		void flush_latestmaxseq() const; // force to flush latest maxseq to disk
		void flush_max_dir_id() const; // force to flush latest maxseq to disk
	private:
		...
		uint64_t max_dir_id; // for directory id
