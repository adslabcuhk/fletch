...
void transaction_main(bool do_server_recovery) {
    ...
    if (server_physical_idx == 0) {  // deploy reflector in the first physical server
        ...
        transaction_expected_ready_threads = 2 + 5 * current_server_logical_num + 3;
    } else {
        transaction_expected_ready_threads = 5 * current_server_logical_num + 3;
    }
    ...
    pthread_t valuedeleteserver_threads[delete_server_number];
    uint16_t valuedeleteserver_params[delete_server_number];
    for (uint16_t valuedeleteserver_i = 0; valuedeleteserver_i < delete_server_number; valuedeleteserver_i++) {
