...
// persistent logs for failure recovery
extern RocksdbWrapper historical_pt;
extern RocksdbWrapper active_pt;
extern RocksdbWrapper historical_ht;
extern RocksdbWrapper active_ht;
RocksdbWrapper historical_pt(NOCACHE_ID);
RocksdbWrapper active_pt(NOCACHE_ID);
RocksdbWrapper historical_ht(NOCACHE_ID);
RocksdbWrapper active_ht(NOCACHE_ID);

// measure switch recovery time
static std::chrono::steady_clock::time_point warmup_start_time;
static std::atomic<bool> warmup_timing_started(false);
static std::atomic<uint64_t> warmup_processed_count(0);
static std::atomic<uint64_t> warmup_total_expected(20000);
