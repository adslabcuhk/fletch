...
void *run_controller_popserver(void *param) {
    ...
    while (controller_running) {
        ...
        printf("[NETFETCH_CONTROLLER] [line %d] receive NETCACHE_CACHE_POP or NETCACHE_CACHE_POP_FINISH from switchos and send it to server\n", __LINE__);
        printf("[NETFETCH_CONTROLLER] [line %d] received buf: ", __LINE__);
        ...
        if (tmp_optype == packet_type_t::NETCACHE_CACHE_POP || tmp_optype == packet_type_t::NETCACHE_PRE_CACHE_POP) {
#ifdef NETFETCH_DEBUG
            printf("[NETFETCH_CONTROLLER] [line %d] receive NETCACHE_CACHE_POP from switchos and send it to server\n", __LINE__);
            printf("[NETFETCH_CONTROLLER] [line %d] recvsize: %d\n", __LINE__, recvsize);
#endif
        ...
        // Qingxiu: debug invariant
#ifdef NETFETCH_DEBUG
        printf("[controller.popserver] [line %d] tmp_serveridx: %d, max_server_total_logical_num = %d\n", __LINE__, tmp_serveridx, max_server_total_logical_num);
#endif
        ...
        // send to another server just for token
        if (server_physical_num > 1) {
            if (tmp_optype == packet_type_t::NETCACHE_CACHE_POP_FINISH) {
                set_sockaddr(tmp_server_popserver_addr, inet_addr(server_ip_for_controller_list[tmp_server_physical_idx ^ 1]), server_popserver_port_start + (tmp_server_physical_idx ^ 1));
...
#ifdef NETFETCH_DEBUG
                printf("[NETFETCH_CONTROLLER] [line %d] send NETCACHE_CACHE_POP_FINISH to another server just for token\n", __LINE__);
                printf("[NETFETCH_CONTROLLER] [line %d] send to another server: %s\n", __LINE__, server_ip_for_controller_list[tmp_server_physical_idx ^ 1]);
                printf("[NETFETCH_CONTROLLER] [line %d] send to another server %d-%d: %d\n", __LINE__, tmp_server_physical_idx, tmp_server_physical_idx ^ 1, server_popserver_port_start + (tmp_server_physical_idx ^ 1));
#endif
                bool is_timeout2 = udprecvfrom(controller_popserver_popclient_udpsock, tmpbuf, MAX_BUFSIZE, 0, NULL, NULL, tmprecvsize, "controller.popserver.popclient");
                if (!is_timeout2) {
                    // send NETCACHE_CACHE_POP_ACK/_FINISH_ACK to switchos.popworker immediately to avoid timeout
                    packet_type_t tmp_acktype = get_packet_type(tmpbuf, recvsize);
#ifdef NETFETCH_DEBUG
                    printf("[NETFETCH_CONTROLLER] [line %d] token prepared\n", __LINE__);
                    printf("[NETFETCH_CONTROLLER] [line %d] recv_size = %d\n", __LINE__, tmprecvsize);
                    printf("[NETFETCH_CONTROLLER] [line %d] received tmprecvsize: ", __LINE__);
                    for (int i = 0; i < tmprecvsize; i++) {
                        printf("%02x ", (unsigned char)(tmpbuf[i]));
                    }
                    printf("\n");
#endif
                } else {
                    printf("[NETFETCH_CONTROLLER] [line %d] timeout\n", __LINE__);
                }
            }
        }
        ...
        if (!is_timeout) {  // NOTE: if timeout, it will be covered by switchos.popworker
            // send NETCACHE_CACHE_POP_ACK/_FINISH_ACK to switchos.popworker immediately to avoid timeout
            ...
            if (tmp_acktype == packet_type_t::NETCACHE_CACHE_POP_ACK) {
                ...
                auto tmp_val = tmp_netcache_cache_pop_ack.val();
                if(tmp_netcache_cache_pop_ack.key() != tmp_key)continue;
                ...
                if(tmp_netcache_cache_pop_ack_nlatest.key() != tmp_key)continue;
                ...
                if(tmp_netcache_cache_pop_finish_ack.key() != tmp_key)continue;
            } else {
