...
void ThreadPool::handle_task(Task* task, int thread_server_fd, int thread_server_invalid_fd, int thread_idx) {
    ...
    case packet_type_t::GETREQ: {
        ...
        if (tmp_val.val_length == FILE_META_SIZE or tmp_val.val_length == DIR_META_SIZE) {
            ...
            // TODO: if the response is for unlocking, then change the op_type to PRE_GETRES
            // TODO: how to determine if the response is for unlocking? use the difference between tmp_keydepth and tmp_real_keydepth
            // TODO: if the difference is more than 0, then it is for unlocking
            // change op_type to PRE_GETRES 0x0019
            if (tmp_real_keydepth > tmp_keydepth) {
                getres_for_unlock = true;
                buf[0] = 0x00;
                buf[1] = 0x19;  // PRE_GETRES
                buf[sizeof(optype_t) + 1] = tmp_keydepth + 1;
            } else {
                getres_for_unlock = false;
            ...
            if (getres_for_unlock) {
                reverse_keys = reverse_key_for_getres_pkt_loss(task->dynamicbuf, task->recv_size, tmp_keydepth, sequence_counter);
            } else {
...
#endif
                        sequence_counter += 1;  // Qingxiu: increase the sequence counter for the next request
                        break;
                    }
...
#ifdef DEBUG_NETFETCH
                // Qingxiu: if it is a normal response (will not be used for unlocking), then no need to wait for the ack from the switch
                // client will trigger re-transmission if it does not receive the response
                printf("[CSFETCH_DEBUG] [line %d] no need to wait for the ack\n", __LINE__);
#endif
                udpsendto(thread_server_fd, buf_getresponse, rsp_size, 0, &task->client_addr, task->client_addrlen, "server.worker");
            }
...
#endif
            if (tmp_keydepth == 2) {
                printf("[CSFETCH_DEBUG] [line %d] tmp_keydepth is 2, which is not expected\n", __LINE__);
