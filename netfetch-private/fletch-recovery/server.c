...
void transaction_main(bool do_server_recovery); // transaction phase
...
int main(int argc, char **argv) {
  ...
  if (argc < 2 || argc > 3) {
    printf("Usage: ./server server_physical_idx [recover|0|1]\n");
    printf("  server_physical_idx: physical server index\n");
    printf("  recover: 1 to trigger server recovery (default 0)\n");
    exit(-1);
  }
  ...
  bool do_server_recovery = false;
  if (argc == 3) {
    std::string flag = argv[2];
    if (flag == "1" || flag == "recover") {
      do_server_recovery = true;
    }
  }
  printf("[main] server_recovery=%s\n",
         do_server_recovery ? "true" : "false");
  ...
  prepare_reflector();
  prepare_server();
  transaction_main(do_server_recovery);
...
void transaction_main(bool do_server_recovery) {
	...
	// server: server_num * (worker + evictserver + popserver + valueupdateserver + valuedeleteserver)
	if (server_physical_idx == 0) { // deploy reflector in the first physical server
		// Qingxiu: modify this since we have new thread for loading freq.
		transaction_expected_ready_threads = 2 + 5*current_server_logical_num;
	}
	else {
		transaction_expected_ready_threads = 5*current_server_logical_num;
	}
	...
	// Qingxiu: launch reflector_cp2dpserver_loadfreq
	// pthread_t reflector_cp2dpserver_loadfreq_thread;
	...
	if (server_physical_idx == 0) {
		...
		// // Qingxiu: launch reflector_cp2dpserver_loadfreq
		// ret = pthread_create(&reflector_cp2dpserver_loadfreq_thread, nullptr, run_reflector_cp2dpserver_loadfreq, nullptr);
		// if (ret) {
		// 	COUT_N_EXIT("Error of launching reflector.cp2dpserver_loadfreq: " << ret);
		// }
		// ret = pthread_setaffinity_np(reflector_cp2dpserver_loadfreq_thread, sizeof(nonserverworker_cpuset), &nonserverworker_cpuset);
		// if (ret) {
		// 	printf("Error of setaffinity for reflector.cp2dpserver_loadfreq; errno: %d\n", errno);
		// 	exit(-1);
		// }
	}
	...
	// launch valuedeleteservers
	pthread_t valuedeleteserver_threads[current_server_logical_num];
	uint16_t valuedeleteserver_params[current_server_logical_num];
	for (uint16_t valuedeleteserver_i = 0; valuedeleteserver_i < current_server_logical_num; valuedeleteserver_i++) {
		valuedeleteserver_params[valuedeleteserver_i] = valuedeleteserver_i;
		ret = pthread_create(&valuedeleteserver_threads[valuedeleteserver_i], nullptr, run_server_valuedeleteserver, &valuedeleteserver_params[valuedeleteserver_i]);
		if (ret) {
		  COUT_N_EXIT("Error of launching some server.valuedeleteserver:" << ret);
		}
		ret = pthread_setaffinity_np(valuedeleteserver_threads[valuedeleteserver_i], sizeof(nonserverworker_cpuset), &nonserverworker_cpuset);
		if (ret) {
			printf("Error of setaffinity for server.valuedeleteserver; errno: %d\n", errno);
			exit(-1);
		}
	}
	...
	// comment it to get clear log for debugging
	...
	if (do_server_recovery){
		...
		printf("[recovery_timing] controller_recovery() took %.2f ms\n",
			recovery_us / 1000.0);
	}
	...
	printf("wait for server.valuedeleteservers\n");
	for (size_t i = 0; i < current_server_logical_num; i++) {
		int rc = pthread_join(valuedeleteserver_threads[i], &status);
		if (rc) {
		  COUT_N_EXIT("Error: unable to join server.valuedeleteserver " << rc);
		}
	}
	printf("server.valuedeleteservers finish\n");
	...
	if (server_physical_idx == 0) {
		...
		// // Qingxiu: join reflector_cp2dpserver_loadfreq
		// printf("wait for reflector.cp2dpserver_loadfreq\n");
		// rc = pthread_join(reflector_cp2dpserver_loadfreq_thread, &status);
		// if (rc) {
		// 	COUT_N_EXIT("Error: unable to join reflector.cp2dpserver_loadfreq " << rc);
		// }
		// printf("reflector.cp2dpserver_loadfreq finish\n");
	}
