...
tbb::concurrent_unordered_map<std::string, token_t> server_cached_path_token_map_list(1000000);
constexpr int SHARD_COUNT = 2048;
struct PairHash {
    std::size_t operator()(const std::pair<netreach_key_t, std::string>& p) const {
#ifdef LARGE_KEY
        size_t hash1 = (p.first.keylolo ^ p.first.keylohi ^ p.first.keyhilo ^ p.first.keyhihi);
#else
...
void prepare_server() {
    ...
    for (size_t i = 0; i < current_server_logical_num; i++) {
        server_netcache_valueupdate_ptr_queue_list[i].init(MQ_SIZE*100);
        uint16_t tmp_global_server_logical_idx = server_logical_idxes_list[server_physical_idx][i];
    ...
    server_valuedeleteserver_udpsock_list = new int[current_server_logical_num];
    for (size_t i = 0; i < current_server_logical_num; i++) {
        server_netcache_valuedelete_ptr_queue_list[i].init(MQ_SIZE*100);
        uint16_t tmp_global_server_logical_idx = server_logical_idxes_list[server_physical_idx][i];
        prepare_udpserver(server_valuedeleteserver_udpsock_list[i], true, server_valueupdateserver_port_start + current_server_logical_num + tmp_global_server_logical_idx, "server.valuedeleteserver");
    }
...
inline std::vector<std::string> extractPathLevels(const std::string& path, int n) {
    ...
    size_t start = 1;  // Start after the initial '/'
    ...
    // Add the remaining part of the path if there is any.
    if (start < path.size()) {
...
inline std::vector<token_t> slowsearchTokensforPaths(const std::vector<std::string>& paths, int local_server_logical_idx) {
    ...
    // return tokens;
#ifdef DEBUG_NETFETCH
    printf("[NETFETCH_DEBUG] [line %d] paths.size(): %d\n", __LINE__, paths.size());
#endif
    int i = 0;
    for (auto& path : paths) {
#ifdef DEBUG_NETFETCH
        printf("[NETFETCH_DEBUG] [line %d] path: %s\n", __LINE__, path.c_str());
#endif
        i++;
...
#ifdef DEBUG_NETFETCH
            printf("[NETFETCH_DEBUG] [line %d] token: %d\n", __LINE__, it->second);
#endif
        } else {
            tokens.push_back(0);
#ifdef DEBUG_NETFETCH
...
void ThreadPool::handle_task(Task* task, int thread_server_fd, int thread_server_invalid_fd, int thread_idx) {
...
#ifdef DEBUG_NETFETCH
    printf("[NETFETCH_DEBUG] [line %d] pkt_type: %d\n", __LINE__, pkt_type);
#endif
    ...
    case packet_type_t::GETREQ: {
...
#ifdef DEBUG_NETFETCH
        printf("[NETFETCH_DEBUG] [line %d] tmp_keydepth: %d, tmp_real_keydepth: %d\n", __LINE__, tmp_keydepth, tmp_real_keydepth);
#endif
        uint16_t path_length;
        ...
        tmp_offset = sizeof(optype_t) + sizeof(keydepth_t) + (tmp_keydepth - 1) * sizeof(netreach_key_t);
        // for (int i = 0; i < sizeof(netreach_key_t); ++i) {
        //     if (digest[i] != (unsigned char)(task->dynamicbuf[i + tmp_offset])) {
        //         printf("[NETFETCH_ERROR] MD5 digest error!\n");
        //         printf("[NETFETCH_ERROR] digest[%d] = %02x, task->dynamicbuf[%d] = %02x\n", i, digest[i],
        //                i + tmp_offset, (unsigned char)(task->dynamicbuf[i + tmp_offset]));
        //     }
        // }
...
#endif
#ifdef DEBUG_NETFETCH
        printf("[NETFETCH_DEBUG] [line %d] tmp_val.metadata_length: %d\n", __LINE__, tmp_val.metadata_length);
#endif
...
#ifdef DEBUG_NETFETCH
        // dump tokens
        printf("[NETFETCH_DEBUG] [line %d] tokens: ", __LINE__);
        for (auto& token : tokens) {
            printf("%d ", token);
        }
        printf("\n");
#endif
        if (tmp_val.metadata_length <= val_t::SWITCH_MAX_VALLEN) {
            ...
            // udpsendlarge_ipfrag(server_worker_udpsock_list[local_server_logical_idx], task->dynamicbuf, rsp_size, 0, &task->client_addr, task->client_addrlen, "server.worker", get_response_largevalue_t::get_frag_hdrsize(CURMETHOD_ID));
        }
        break;
    }
    ...
    case packet_type_t::PUTREQ_LARGEVALUE_SEQ: {
        ...
        if (pkt_type == packet_type_t::PUTREQ_SEQ) {
            ...
            uint16_t tmp_keydepth = get_keydepth(task->dynamicbuf, task->recv_size);
#ifdef DEBUG_NETFETCH
            printf("[NETFETCH_DEBUG] [line %d] tmp_keydepth: %d\n", __LINE__, tmp_keydepth);
#endif
            ...
            // layout: op_type + keydepth + keys + value_length + value + shadowtype + seq + path_length
...
#ifdef DEBUG_NETFETCH
            printf("[NETFETCH_DEBUG] [line %d] PUTREQ bring_token: %d\n", __LINE__, bring_token);
#endif
        ...
        } else if (pkt_type == packet_type_t::PUTREQ_LARGEVALUE_SEQ) {  // not used in fletchrec
...
#ifdef DEBUG_NETFETCH
        printf("[NETFETCH_DEBUG] [line %d] start check key set\n", __LINE__);
#endif
        bool tmp_stat = false;
        ...
        if (likely(!is_being_cached && !is_cached)) {  // uncached
#ifdef DEBUG_NETFETCH
            printf("[NETFETCH_DEBUG] [line %d] check key set -> uncached\n", __LINE__);
#endif
            if (pkt_type == packet_type_t::PUTREQ_SEQ || pkt_type == packet_type_t::PUTREQ_LARGEVALUE_SEQ) {
                ...
                if (strcmp(tmp_val.val_data, "touch") == 0) {
                    ...
                    if (result_pointer) {
                        DBGprint("[NETFETCH_DEBUG] [line %d] hdfsOpenFile successfully!\n", __LINE__);
                        hdfsCloseFile(ns1, result_pointer);
                        result_pointer = NULL;
                    } else {
                        DBGprint("[NETFETCH_ERROR] [line %d] hdfsOpenFile failed!\n", __LINE__);
                ...
                // delete file
                    ...
                    if (result == 0) {
#ifdef DEBUG_NETFETCH
                        printf("[NETFETCH_DEBUG] [line %d] hdfsDelete successfully!\n", __LINE__);
#endif
                    } else {
#ifdef DEBUG_NETFETCH
                        printf("[NETFETCH_ERROR] [line %d] hdfsDelete failed!\n", __LINE__);
#endif
                        failure_flag = 1;
                    }
                }
                // create directory
                    ...
                    if (result == 0) {
#ifdef DEBUG_NETFETCH
                        printf("[NETFETCH_DEBUG] [line %d] hdfsCreateDirectory successfully!\n", __LINE__);
#endif
                    } else {
#ifdef DEBUG_NETFETCH
                        printf("queried_path: %s\n", queried_path);
                        printf("[NETFETCH_ERROR] [line %d] hdfsCreateDirectory failed!\n", __LINE__);
#endif
                        failure_flag = 1;
                    }
                }
                ...
                // chmod for file or directory
                    ...
                    if (mode != -1) {  // Ensure the mode was correctly extracted
                        ...
                        if (result == 0) {
                            failure_flag = 0;
#ifdef DEBUG_NETFETCH
                            printf("[NETFETCH_DEBUG] [line %d] hdfsChmod successfully applied permissions %o on %s\n", __LINE__, mode, queried_path);
#endif
                        } else {
#ifdef DEBUG_NETFETCH
                            printf("[NETFETCH_ERROR] [line %d] hdfsChmod failed on %s with mode %o\n", __LINE__, queried_path, mode);
#endif
                            failure_flag = 1;
                        }
                    } else {
                ...
                // mv for file or directory
                    ...
                    if (result == 0) {
#ifdef DEBUG_NETFETCH
                        printf("[NETFETCH_DEBUG] [line %d] hdfsRename %s to %s successfully!\n", __LINE__, queried_path, new_path);
#endif
                    } else {
#ifdef DEBUG_NETFETCH
                ...
                //     batchMaxDuration = 0;
                // }
            } else {  // perform DEL operation, not use in fletchrec
            }
        } else if (unlikely(is_being_cached)) {  // being cached
#ifdef DEBUG_NETFETCH
            printf("[NETFETCH_DEBUG] [line %d] check key set -> being cached\n", __LINE__);
#endif
            INVARIANT(!is_cached);
...
#ifdef DEBUG_NETFETCH
            printf("[NETFETCH_DEBUG] [line %d] check key set -> already cached\n", __LINE__);
#endif
...
#ifdef DEBUG_NETFETCH
                printf("[NETFETCH_DEBUG] [line %d] PUTREQ cached real_token: %d\n", __LINE__, real_token);
#endif
                if (real_token != bring_token) {
#ifdef DEBUG_NETFETCH
                    printf("[NETFETCH_DEBUG] [line %d] invalid the cache. client take wrong token\n", __LINE__);
#endif
...
#ifdef DEBUG_NETFETCH
                        printf("[NETFETCH_POPSERVER_DEBUG] [line %d] set invalid pktsize %d send_buf: ", __LINE__, pktsize);
                        for (int i = 0; i < pktsize; ++i) {
...
#ifdef DEBUG_NETFETCH
                    printf("[NETFETCH_DEBUG] [line %d] invalided the cache.\n", __LINE__);
#endif
                }
                        ...
                        if (strncmp(tmp_val.val_data, "chmod", 5) == 0) {
                            ...
                            if (failure_flag == 0 || failure_flag == 1) {
                                if (failure_flag == 0) {  // success
                                    // fetch updated metadata from hdfs
                                    fetch_metadata_from_hdfs(tmp_metadata, queried_path, 3);
                                    tmp_val = val_t(tmp_metadata.metadata_data + sizeof(uint16_t), tmp_metadata.metadata_length);
                                } else {  // failure
                                    tmp_val = val_t();
                                    std::cout << "[NETFETCH_ERROR] chmod failed on " << queried_path << std::endl;
                                    exit(-1);
                                }
        ...
        if (pkt_type == packet_type_t::PUTREQ_SEQ || pkt_type == packet_type_t::PUTREQ_LARGEVALUE_SEQ) {
            ...
            if (failure_flag == 1) {
                ...
                uint8_t failure_flag_for_reqponse = 0xFF;
                memcpy(&buf[tmp_offset], &failure_flag_for_reqponse, sizeof(uint8_t));
            }
        ...
        rsp_size += tokens.size();
#ifdef DEBUG_NETFETCH
        printf("[NETFETCH_DEBUG] [line %d] already send put response to client", __LINE__);
#endif
    ...
    case packet_type_t::PUTREQ_LARGEVALUE_SEQ_CACHED: {
        ...
        std::string queried_path_str;
        char new_path[MAX_PATH_LENGTH];  // for mv only
        // Qingxiu: modify the value to metadata
        // Qingxiu: only modify val to file system metadata
        // Qingxiu: before modifying value, we need to do filesystem operation and fetch metadata from file system
        if (pkt_type == packet_type_t::NETCACHE_PUTREQ_SEQ_CACHED) {
            uint16_t tmp_keydepth = get_keydepth(task->dynamicbuf, task->recv_size);
#ifdef DEBUG_NETFETCH
            printf("[NETFETCH_DEBUG] tmp_keydepth: %d\n", tmp_keydepth);
#endif
...
#ifdef DEBUG_NETFETCH
        printf("[NETFETCH_DEBUG] [line %d] is_being_cached: %d, is_cached: %d, is_being_updated: %d\n", __LINE__, is_being_cached, is_cached, is_being_updated);
#endif
        ...
        if (likely(is_cached)) {  // already cached
            ...
            }  // perform DEL operation, never use in fletchrec
        }
        if (is_skip_invalid && !is_create) {
        } else if (is_create) {
            shard_mutexes[shard_index].unlock();
...
#ifdef DEBUG_NETFETCH
        printf("[NETFETCH_DEBUG] [line %d] already send put response to client!\n", __LINE__);
#endif
        udpsendto(thread_server_fd, buf, rsp_size, 0, &task->client_addr, task->client_addrlen, "server.worker");
        break;
    }
...
#ifdef LARGE_KEY
            printf("[server.evictserver %d-%d ERROR] evicted key %x is not in beingcached keyset (size: %d) or cached keyset (size: %d)\n", local_server_logical_idx, global_server_logical_idx, tmp_netcache_cache_evict.key().keyhihi, beingcached_set.size(), cached_set.size());
#else
            // bool is_found = false;
    ...
    while (transaction_running) {
        ...
        if (it != NULL) {
            ...
            if (failure_flag == 0) {  // update cache value
                ...
                printf("[NETFETCH_DEBUG] [line %d] sendbuf: ", __LINE__);
                dump_buf(sendbuf, sendsize);
#endif
...
void* run_server_valuedeleteserver(void* param) {
    ...
    while (transaction_running) {
        auto it = server_netcache_valuedelete_ptr_queue_list[local_server_logical_idx].read();
        if (it != NULL) {
            ...
            bool is_being_updated = (beingupdated_set.find(check_key) != beingupdated_set.end());
#ifdef DEBUG_NETFETCH
            printf("[NETFETCH_DEBUG] [line %d] is_being_updated: %d\n", __LINE__, is_being_updated);
#endif
            if (likely(is_being_updated)) {
                beingupdated_set.erase(check_key);
            } else {  // due to cache eviciton
...
void server_recovery() {
    const int recovery_port = 9999;
    const int MAX_REQUEST_RETRIES = 100;

    // ---- create UDP client socket ----
    int sockfd;
    create_udpsock(sockfd, true, "server.server_recovery",
                   0, SERVER_SOCKET_TIMEOUT_USECS);

    struct sockaddr_in controller_addr;
    set_sockaddr(controller_addr, inet_addr(switchos_ip), recovery_port);
    ...
    // ---- parse helper ----
    // wire format of each packet from controller:
    //   [uint32 entry_id][uint32 total][uint16 path_len][path][uint8 token]
    auto parse_and_insert = [&](const char* buf, int n, uint32_t& out_id,
                                uint32_t& out_total) -> bool {
        if (n < 11) return false;  // 4 + 4 + 2 + 0 + 1
        const char* p = buf;
        memcpy(&out_id, p, 4); p += 4;
        memcpy(&out_total, p, 4); p += 4;
        uint16_t path_len;
        memcpy(&path_len, p, 2); p += 2;
        if (n < (int)(10 + path_len + 1)) return false;
        std::string path(p, path_len);
        p += path_len;
        uint8_t token = static_cast<uint8_t>(*p);
        server_cached_path_token_map_list[path] = token;
        return true;
    };

    // ---- send request, retry if no response ----
    const char* request_msg = "RECOVER";
    int request_len = strlen(request_msg);
    uint32_t total = 0;
    uint32_t first_id = 0;

    int retry = 0;
    while (true) {
        udpsendto(sockfd, request_msg, request_len, 0, &controller_addr,
                  controller_addr_len, "server.server_recovery");
        printf("[server_recovery] sent recovery request to controller\n");

        is_timeout = udprecvfrom(sockfd, recvbuf, MAX_BUFSIZE, 0, NULL, NULL,
                                 recvsize, "server.server_recovery");
        if (!is_timeout && parse_and_insert(recvbuf, recvsize, first_id,
                                            total)) {
            break;
        }
        ...
        if (retry >= MAX_REQUEST_RETRIES) {
            printf("[server_recovery] exceeded max retries, giving up\n");
            close(sockfd);
            return;
        }
        // printf("[server_recovery] no response, resending request (retry %d)\n", retry);
    }

    // printf("[server_recovery] expecting %u entries\n", total);

    // ACK for first entry
    memcpy(sendbuf, &first_id, 4);
    udpsendto(sockfd, sendbuf, 4, 0, &controller_addr, controller_addr_len,
              "server.server_recovery");

    // ---- receive remaining entries, ACK each ----
    std::set<uint32_t> received_ids;
    received_ids.insert(first_id);

    while (received_ids.size() < total) {
        is_timeout = udprecvfrom(sockfd, recvbuf, MAX_BUFSIZE, 0, NULL, NULL,
                                 recvsize, "server.server_recovery");
        if (is_timeout) {
            continue;  // controller will resend; keep waiting
        }

        uint32_t this_id = 0, this_total = 0;
        if (!parse_and_insert(recvbuf, recvsize, this_id, this_total)) {
            printf("[server_recovery] malformed packet, skipping\n");
            continue;
        }

        // always ACK, even for duplicates
        memcpy(sendbuf, &this_id, 4);
        udpsendto(sockfd, sendbuf, 4, 0, &controller_addr, controller_addr_len,
                  "server.server_recovery");

        received_ids.insert(this_id);
    }

#ifdef DEBUG_NETFETCH
    printf("[server_recovery] done, recovered %zu entries\n",
           server_cached_path_token_map_list.size());

    // ---- print the recovered map ----
    printf("[server_recovery] server_cached_path_token_map_list:\n");
    for (const auto& kv : server_cached_path_token_map_list) {
