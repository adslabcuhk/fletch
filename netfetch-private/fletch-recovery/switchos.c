...
// #define NETFETCH_POPSERVER_DEBUG
...
void* run_switchos_popserver(void* param) {
    ...
    while (switchos_running) {
        ...
        if (tmp_optype == packet_type_t::NETCACHE_WARMUPREQ_INSWITCH_POP) {
            bool expected = false;
            if (warmup_timing_started.compare_exchange_strong(expected, true)) {
                warmup_start_time = std::chrono::steady_clock::now();
                // printf("[warmup_timing] start at first warmup packet\n");
            }
        ...
        if (tmp_key_path_mapping_ptr != NULL) {
            if (tmp_key_path_mapping_ptr->space_consumption > 0) {  // cache pop
                ...
                if (tmp_key_path_mapping_ptr->space_consumption > 1) {
                    tmp_frequency = 10000;                                                       // default frequency for hot reported path by CM sketch
                ...
                // file space_consumption * 0.6 + 0.4
                double tmp_scaled_frequency = 10000.0;
                if (tmp_val.back().val_length == file_metadata_length) {
                    tmp_scaled_frequency = static_cast<double>(tmp_frequency) ;
                } else {
                    tmp_scaled_frequency = static_cast<double>(tmp_frequency) ;
                }
                // dir space_consumption*0.6
#ifdef NETFETCH_POPSERVER_DEBUG
                ...
                if (free_idxes.size() == tmp_key_path_mapping_ptr->space_consumption) {                                                       // with free idx
                    ...
                    std::vector<std::tuple<std::string, double, int>> potential_victims = findKVictims(  // int for space consumption
                        sorted_path_scaled_frequency_space_tuples, number_of_victims, is_file_admission,
                ...
                warmup_processed_count++;
                if (warmup_processed_count == warmup_total_expected) {
                    auto end_time = std::chrono::steady_clock::now();
                    auto elapsed_us = std::chrono::duration_cast<std::chrono::microseconds>(
                        end_time - warmup_start_time).count();
                    printf("[warmup_timing] all %lu warmup admissions done in (%.2f ms)\n",
                        warmup_total_expected.load(), elapsed_us / 1000.0);
                    fflush(stdout);
                }
...
int main(int argc, char** argv) {
    ...
    bool do_controller_recovery = false;
    for (int i = 1; i < argc; i++) {
        std::string arg = argv[i];
        if (arg == "--recover" || arg == "-r") {
            do_controller_recovery = true;
        } else if (arg == "--help" || arg == "-h") {
            printf("Usage: %s [--recover|-r]\n", argv[0]);
            printf("  --recover, -r   Recover controller's in-memory maps from historical log\n");
            return 0;
        }
    }
    ...
    bool is_historical = 1;
    bool is_active = 0;
    historical_pt.open_path_map(is_historical);
    active_pt.open_path_map(is_active);
    historical_ht.open_hash_map(is_historical);
    active_ht.open_hash_map(is_active);
    ...
    // dump_active_paths_for_warmup("active_path.log");
    ...
    pthread_t server_recovery_thread;
    ret = pthread_create(&server_recovery_thread, nullptr, server_recovery, nullptr);
    if (ret) {
        COUT_N_EXIT("Error: " << ret);
    }
    ...
    rc = pthread_join(server_recovery_thread, &status);
    if (rc) {
        COUT_N_EXIT("Error:unable to join," << rc);
    }
