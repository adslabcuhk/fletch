...
std::pair<uint32_t, int> find_logical_physical_server_idx_for_file(std::string path) {
    ...
    if (Patch::patch_mode != 1) {
        ...
        tmp_server_physical_idx = 0;
        tmp_global_server_logical_idx = 0;
    }
...
void generate_tokens_for_newly_admitted_paths(int number_of_paths_for_admission, std::vector<uint16_t> tmp_pipeidxes, std::vector<netreach_key_t> admitted_keys) {
    ...
    for (int i = 0; i < number_of_paths_for_admission; ++i) {
        ...
        if (switchos_cached_path_token_map.find(newly_admitted_paths[i]) != switchos_cached_path_token_map.end()) {
            ...
            // // persist to active log only (historical already has it)
            // uint64_t hash = (static_cast<uint64_t>(admitted_keys[i].keylo) << 32) | 
            //     static_cast<uint64_t>(__builtin_bswap32(admitted_keys[i].keyhi));
            // // printf("hash = %016lx\n", hash);
            // active_pt.put_path_token(newly_admitted_paths[i], assign_token);
            // active_ht.put_hash_token(hash, assign_token);
        ...
        // // persist to both historical and active logs
        // uint64_t hash = (static_cast<uint64_t>(admitted_keys[i].keylo) << 32) | 
        //         static_cast<uint64_t>(__builtin_bswap32(admitted_keys[i].keyhi));
        // // printf("hash = %016lx\n", hash);
        // active_pt.put_path_token(newly_admitted_paths[i], assign_token);
        // active_ht.put_hash_token(hash, assign_token);
        // historical_pt.put_path_token(newly_admitted_paths[i], assign_token);
        // historical_ht.put_hash_token(hash, assign_token);
...
void controller_recovery() {
    // ---- recover path-token map from historical_pt log ----
    std::vector<std::pair<std::string, uint8_t>> path_tokens;
    historical_pt.get_all_path_tokens(path_tokens);
    switchos_cached_path_token_map.clear();
    for (auto& kv : path_tokens) {
        switchos_cached_path_token_map[kv.first] = kv.second;
    }
#ifdef NETFETCH_POPSERVER_DEBUG
    printf("[controller_recovery] recovered %zu path-token entries\n", path_tokens.size());
#endif

#ifdef NETFETCH_POPSERVER_DEBUG
    // ---- print recovered path-token map ----
    printf("[controller_recovery] in-memory path-token map:\n");
    for (auto& kv : switchos_cached_path_token_map) {
        printf("    %s -> token=%d\n", kv.first.c_str(), (int)kv.second);
    }
#endif

    // ---- recover hash-token mapset from historical_ht log ----
    std::vector<std::pair<uint64_t, uint8_t>> hash_tokens;
    historical_ht.get_all_hash_tokens(hash_tokens);
    switchos_cached_key_token_mapset.clear();
    for (auto& kv : hash_tokens) {
        uint64_t hash = kv.first;
        uint8_t token = kv.second;

        // reconstruct netreach_key_t from uint64 hash
        // (reverse of the encoding done in generate_tokens_for_newly_admitted_paths)
        uint32_t keylo = static_cast<uint32_t>(hash >> 32);
        uint32_t hi_fixed = static_cast<uint32_t>(hash & 0xFFFFFFFF);
        // reverse the half-word byte swap
        uint32_t keyhi = ((hi_fixed << 8) & 0xFF00FF00) |
                         ((hi_fixed >> 8) & 0x00FF00FF);

        netreach_key_t key(keylo, keyhi);
        switchos_cached_key_token_mapset[key].insert(token);
    }
#ifdef NETFETCH_POPSERVER_DEBUG
    printf("[controller_recovery] recovered %zu hash-token entries\n", hash_tokens.size());
    // ---- print recovered hash-token mapset ----
    printf("[controller_recovery] in-memory hash-token mapset:\n");
    for (auto& kv : switchos_cached_key_token_mapset) {
        // reconstruct the stored uint64 hash for display (same as what we stored)
        uint32_t keylo = kv.first.keylo;
        uint32_t keyhi = kv.first.keyhi;
        uint32_t hi_fixed = ((keyhi << 8) & 0xFF00FF00) |
                            ((keyhi >> 8) & 0x00FF00FF);
        uint64_t hash = (static_cast<uint64_t>(keylo) << 32) |
                        static_cast<uint64_t>(hi_fixed);
        printf("    hash=%016lx tokens={", hash);
    ...
    printf("[controller_recovery] done\n");
#endif
}

void* server_recovery(void* arg) {
    const int recovery_port = 9999;
    const int RECV_TIMEOUT_USEC = 1000000;  // 1 second

    int sockfd;
    prepare_udpserver(sockfd, false, recovery_port, "switchos.server_recovery",
                      0, RECV_TIMEOUT_USEC);
    printf("[server_recovery] listening on UDP port %d\n", recovery_port);
    ...
    while (true) {
        // ---- wait for a recovery request from server ----
        struct sockaddr_in server_addr;
        socklen_t server_addr_len = sizeof(server_addr);
        ssize_t req_n = recvfrom(sockfd, pktbuf, MAX_BUFSIZE, 0,
                                 (struct sockaddr*)&server_addr,
                                 &server_addr_len);
        if (req_n < 0) {
            if (errno == EAGAIN || errno == EWOULDBLOCK) continue;
            printf("[server_recovery] recvfrom error: %s\n", strerror(errno));
            continue;
        }

        char client_ip[INET_ADDRSTRLEN];
        inet_ntop(AF_INET, &server_addr.sin_addr, client_ip,
                  sizeof(client_ip));

#ifdef NETFETCH_POPSERVER_DEBUG
        printf("[server_recovery] got recovery request from %s:%d\n",
        ...
        // ---- read path-token map from active log ----
        std::vector<std::pair<std::string, uint8_t>> path_tokens;
        active_pt.get_all_path_tokens(path_tokens);
        uint32_t total = static_cast<uint32_t>(path_tokens.size());

#ifdef NETFETCH_POPSERVER_DEBUG
        printf("[server_recovery] sending %u entries\n", total);
#endif

        // ---- send each entry and wait for its ACK, retransmit on timeout ----
        for (uint32_t i = 0; i < total; i++) {
            const std::string& path = path_tokens[i].first;
            uint8_t token = path_tokens[i].second;
            uint16_t path_len = static_cast<uint16_t>(path.size());

            // wire format:
            //   [uint32 entry_id][uint32 total][uint16 path_len][path][uint8 token]
            std::vector<char> sendbuf;
            sendbuf.insert(sendbuf.end(), (char*)&i, (char*)&i + 4);
            sendbuf.insert(sendbuf.end(), (char*)&total, (char*)&total + 4);
            sendbuf.insert(sendbuf.end(), (char*)&path_len,
                           (char*)&path_len + 2);
            sendbuf.insert(sendbuf.end(), path.begin(), path.end());
            sendbuf.insert(sendbuf.end(), (char*)&token, (char*)&token + 1);

            // retransmit until ACK for entry_id == i is received
            while (true) {
                ssize_t sent = sendto(sockfd, sendbuf.data(), sendbuf.size(),
                                      0, (struct sockaddr*)&server_addr,
                                      server_addr_len);
                if (sent < 0) {
                    printf("[server_recovery] sendto error: %s\n",
                           strerror(errno));
                }
                ...
                bool got_ack = false;
                while (true) {
                    ssize_t ack_n = recvfrom(sockfd, ackbuf, MAX_BUFSIZE, 0,
                                             NULL, NULL);
                    if (ack_n < 0) {
                    ...
                    if (ack_n >= 4) {
                        uint32_t ack_id;
                        memcpy(&ack_id, ackbuf, 4);
                        if (ack_id == i) {
                            got_ack = true;
                            break;
                        }
                        // ignore unmatched (duplicate) ACKs
                    }
                }

                if (is_timeout) {
                    printf("[server_recovery] timeout, retransmit entry %u\n",
                           i);
                    continue;
                }
                if (got_ack) break;
        ...
        printf("[server_recovery_timing] sent %u entries to %s:%d in %.2f ms\n",
               total, client_ip, ntohs(server_addr.sin_port),
               recovery_us / 1000.0);
        fflush(stdout);

#ifdef NETFETCH_POPSERVER_DEBUG
        printf("[server_recovery] done sending to %s:%d\n", client_ip,
...
void dump_active_paths_for_warmup(const std::string& out_file) {
    std::vector<std::pair<std::string, uint8_t>> path_tokens;
    active_pt.get_all_path_tokens(path_tokens);
    ...
    fout.close();
    printf("[recovery] exported %zu paths to %s\n",
           path_tokens.size(), out_file.c_str());
}
