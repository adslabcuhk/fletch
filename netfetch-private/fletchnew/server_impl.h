...
bool getres_for_unlock[20]; // for getres for unlock (one socket has one bool)
int sequence_counter[20]; // for getres for unlock (one socket has one sequence counter)
...
void ThreadPool::handle_task(Task* task, int thread_server_fd, int thread_server_invalid_fd, int thread_idx) {
    ...
    case packet_type_t::GETREQ: {
        ...
        if (tmp_val.metadata_length <= val_t::SWITCH_MAX_VALLEN) {
            ...
            if (tmp_real_keydepth > tmp_keydepth) { // means that this request is partial hit and requires the response pkt for unlocking
                getres_for_unlock[thread_idx] = true;
                buf[0] = 0x00;
                buf[1] = 0x19;  // PRE_GETRES
                buf[sizeof(optype_t) + 1] = tmp_keydepth + 1;
            } else {
                getres_for_unlock[thread_idx] = false;
            ...
            std::vector<char> reverse_keys;
            if (getres_for_unlock[thread_idx]) {
                reverse_keys = reverse_key_for_getres_pkt_loss(task->dynamicbuf, task->recv_size, tmp_keydepth, sequence_counter[thread_idx]);
            } else {
                reverse_keys = reverse_key_for_getres(task->dynamicbuf, task->recv_size, tmp_keydepth);
            }
            ...
            printf("\n");
#endif
            if (buf_getresponse[0] == 0x00 && buf_getresponse[1] == 0x19) {
                while (true) {
                    udpsendto(thread_server_fd, buf_getresponse, rsp_size, 0, &task->client_addr, task->client_addrlen, "server.worker");
                    // wait for the ack from the switch
                    bool is_timeout = false;
                    is_timeout = udprecvfrom(thread_server_fd, ackbuf, MAX_BUFSIZE, 0, NULL, NULL, ack_recvsize, "server.worker");
                    if (unlikely(is_timeout)) {
                        continue;
                    }
                    // check if the ack is valid
                    if (ackbuf[0] == 0x00 && ackbuf[1] == 0x59) {
#ifdef DEBUG_NETFETCH
                        // Qingxiu: it is an ack for unlock, no packet loss happens
                        printf("[CSFETCH_DEBUG] [line %d] ACK received, no packet loss\n", __LINE__);
                        printf("[FLETCH_DEBUG] [line %d] thread_idx = %d\n", __LINE__, thread_idx);
                        printf("[FLETCH_DEBUG] [line %d] counter_index = %d\n", __LINE__, thread_idx);
#endif
                        sequence_counter[thread_idx] += 1;  // Qingxiu: increase the sequence counter for the next request
                        break;
                    }
                    else if (ackbuf[0] == 0x00 && ackbuf[1] == 0x69) {
#ifdef DEBUG_NETFETCH
                        printf("[CSFETCH_DEBUG] [line %d] ACK received, packet loss happens\n", __LINE__);
                        printf("[FLETCH_DEBUG] [line %d] thread_idx = %d\n", __LINE__, thread_idx);
                        printf("[FLETCH_DEBUG] [line %d] counter_index = %d\n", __LINE__, thread_idx);
#endif
                        sequence_counter[thread_idx] += 1;  // Qingxiu: increase the sequence counter for the next request
...
#ifdef DEBUG_NETFETCH
                        printf("[CSFETCH_DEBUG] [line %d] invalid ack received, ackbuf[0]: %02x, ackbuf[1]: %02x\n", __LINE__, ackbuf[0], ackbuf[1]);
#endif
                        continue;
                    }
                }
