...
def _bfshell_connect():
    import paramiko
    switch203_host = "10.26.43.200"
    switch203_user = "cyh"
    switch203_password = "er1ahht41q"
    switch203_port = 43177
    bfshell_bin = "/home/cyh/bf-sde-9.2.0/install/bin/bfshell"

    client = paramiko.SSHClient()
    client.set_missing_host_key_policy(paramiko.AutoAddPolicy())
    client.connect(switch203_host, port=switch203_port, username=switch203_user,
                   password=switch203_password, timeout=10)
    ...
    channel.send(f"{bfshell_bin}\n")
    wait_for_prompt("bf-sde>", timeout=10)
    channel.send("ucli\n")
    wait_for_prompt("bf-sde.ucli>", timeout=5)
    channel.send("pm\n")
    wait_for_prompt("bf-sde.pm>", timeout=5)

    return client, channel, wait_for_prompt


def _bfshell_disconnect(client, channel):
    channel.send("exit\n"); time.sleep(0.5)
    channel.send("exit\n"); time.sleep(0.5)
    channel.send("exit\n"); time.sleep(0.5)
    channel.close()
...
def clr_port5_stats(logger: logging.Logger):
    try:
        client, channel, wait_for_prompt = _bfshell_connect()
        channel.send("port-stats-clr 5/0\n")
        wait_for_prompt("bf-sde.pm>", timeout=5)
        _bfshell_disconnect(client, channel)
        logger.info("[switch203] port 5/0 stats cleared")
    except Exception as e:
        logger.error(f"[switch203] Failed to clear port 5/0 stats: {e}")


def get_port5_rx(logger: logging.Logger) -> Optional[int]:
    import re
    try:
        client, channel, wait_for_prompt = _bfshell_connect()
        channel.send("show\n")
        output = wait_for_prompt("bf-sde.pm>", timeout=10)
        _bfshell_disconnect(client, channel)
        ...
        if match:
            rx_frames = int(match.group(1))
            logger.info(f"[switch203] port 5/0 FRAMES RX = {rx_frames}")
            return rx_frames
        else:
            logger.warning(f"[switch203] Could not parse port 5/0 RX from output:\n{output}")
            return None
    except Exception as e:
        logger.error(f"[switch203] Failed to get port 5/0 RX: {e}")
        return None
                    ...
                    for zeta in zetas:
                        ...
                        for current_method in current_methods:
                            ...
                            for workload_generation_method in workload_generation_methods:
                                ...
                                rotation_list= [i for i in range(server_logical_num)]
                                # rotation_list= [0, 1, 2, 3, 4, 5, 6, 13, 14, 15]
                                # rotation_list = [i for i in range(13, server_logical_num)]
                                ...
                                while rotation_idx < len(rotation_list):
                                    ...
                                    if bottleneck_hit is None:
                                        if is_bottleneck and bottleneck_id == rotation_list[0]:
                                        ...
                                        if need_retry:
                                            if is_bottleneck:
                                                if retry_idx > max_retry:
                                                    logger.fatal(f"Bottleneck {rotation_list[rotation_idx]} fail, max retry {max_retry} reached. Next Workload")
                                                    # exit while loop and go to next workload
                                                    break
                                                if bottleneck_hit > 1e4 and rotation_hit == 0:
                                                    logger.info(f"bottleneck {rotation_list[rotation_idx]} success with {retry_idx} retry")
                                                    get_port5_rx(logger)
                                                    rotation_idx += 1
                                                    retry_idx = 0
                                                else:
                                                    retry_idx += 1
                                                    if bottleneck_id == 0:
                                                        logger.error("Bottleneck 0 fail, restarting entire testbed")
                                                        stop_all()
                                                        start_testbed(method_name, depth)
                                                    else:
                                                        logger.error(f"Bottleneck {bottleneck_id} fail, retrying current rotation without restart")
                                            else:
                                                if retry_idx > max_retry:
                                                    logger.fatal(f"rotation {rotation_list[rotation_idx]} fail, max retry {max_retry} reached. Next rotation")
                                                    rotation_idx += 1
                                                    retry_idx = 0
                                                    # continue while loop and go to next rotation
                                                    continue
                                                if bottleneck_hit > 1e4 and rotation_hit > 0:
                                                    logger.info(f"rotation {rotation_list[rotation_idx]} success with {retry_idx} retry")
                                                    rotation_idx += 1
                                                    retry_idx = 0
                                                else:
                                                    logger.error("rotation fail, need to retry rotation")
