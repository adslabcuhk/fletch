...
void mdtest_write_hot_paths_to_switch(std::string &path, bool is_file_test = true) {
    ...
    if (current_method == NETFETCH_ID || current_method == NETFETCH_ID_1 || current_method == NETFETCH_ID_2 || current_method == NETFETCH_ID_3 || current_method == CSFLETCH_ID || current_method == FLETCH_RECOVERY_ID
        || current_method == CSFLETCH_RECOVERY_ID) {
        if (is_uniform) {
...
void prepare_hot_path_token(std::string &path) {
    if (!(current_method == NETFETCH_ID || current_method == NETFETCH_ID_3 || current_method == CSFLETCH_ID || current_method == FLETCH_RECOVERY_ID || current_method == CSFLETCH_RECOVERY_ID)) {
        return;
    }
    int kv_count = hot_path_size;  // Assuming a defined kv_count value
...
#endif
        if (!(current_method == NOCACHE_ID || current_method == CSCACHE_ID) && ((mode == 1 && bottleneck_id == 0) || mode == 0 || need_warm == true)) {
...
int main(int argc, char **argv) {
    ...
    if (is_mixed == 1 && create_only == 1) {  // create + mkdir
        ...
        if (!(current_method == NOCACHE_ID || current_method == CSCACHE_ID) && ((mode == 1 && bottleneck_id == 0) || mode == 0 || need_warm == true)) {
            ...
            // dump_reqs_to_file("reqs_mixed.txt");
            mdtest_write_hot_paths_to_switch(temp_path);
            // sleep for tofino ready
            if ((current_method == NETFETCH_ID || current_method == CSFLETCH_ID || current_method == FLETCH_RECOVERY_ID || current_method == CSFLETCH_RECOVERY_ID) && mode == 0) {
        ...
        if (current_method == CSCACHE_ID || current_method == CSFLETCH_ID || current_method == CSFLETCH_RECOVERY_ID) {
            prepare_client_cache(testdir);
        }
