...
void dump_reqs_to_file(const std::string &filename) {
    std::ofstream f(filename);
    if (!f.is_open()) {
        std::cerr << "Error: cannot open dump file " << filename << std::endl;
        return;
    }
    if (!reqs_mixed.empty()) {
        // mixed workload: op + path
        for (const auto &[op, path, nodeidx] : reqs_mixed) {
            f << static_cast<int>(op) << " " << path << "\n";
        }
    }
    f.close();
    std::cout << "Dumped " << (reqs_mixed.empty() ? reqs.size() : reqs_mixed.size())
              << " requests to " << filename << std::endl;
}
