...
void ThreadPool::handle_task(Task* task, int thread_server_fd, int thread_server_invalid_fd, int thread_idx) {
    ...
    case packet_type_t::GETREQ: {
        ...
        if (tmp_val.metadata_length <= val_t::SWITCH_MAX_VALLEN) {
            ...
            // get second paths's keys md5 hash and get the the 7th and 8th bytes
            unsigned char second_digest[MD5_DIGEST_LENGTH];
            if (split_paths.size() > 1) {
                MD5((unsigned char*)split_paths[1].c_str(), split_paths[1].size(), second_digest);
            } else {
                memset(second_digest, 0, MD5_DIGEST_LENGTH);
            }
            char* tail = buf_getresponse + rsp_size;
            // write the 7th and 8th bytes into the response
            // localtion tail - 5
            memcpy(tail - 5, second_digest + 6, 2);
