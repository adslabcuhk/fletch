...
#ifdef NETFETCH_POPSERVER_DEBUG
    std::cout << "[" << __LINE__ << "] evicted_count: " << evicted_count
              << ", number_of_evicted_paths: " << number_of_evicted_paths
              << ", sorted_indexes.size()" << sorted_indexes.size() << std::endl;
#endif
    if (!has_file_evicted && is_file_admission) {
...
#ifdef NETFETCH_POPSERVER_DEBUG
                printf("[POPSERVER] [line %d] potential_victims[%d]: %s, %f, %d\n", __LINE__, i,
                       std::get<0>(potential_victims[sorted_indexes[i]]).c_str(),
                       std::get<1>(potential_victims[sorted_indexes[i]]),
                       std::get<2>(potential_victims[sorted_indexes[i]]));
#endif
                if (std::get<2>(potential_victims[sorted_indexes[i]]) > 1 && (number_of_evicted_paths - evicted_count) > 1) {
                    std::vector<std::string> internal_paths = get_internal_paths(std::get<0>(potential_victims[sorted_indexes[i]]));
                    size_t number_of_evicted_internal_path = std::min(number_of_evicted_paths - evicted_count - 1, std::get<2>(potential_victims[sorted_indexes[i]]) - 1);
...
std::vector<std::tuple<std::string, double, int>> old_findKVictims(
    std::list<std::tuple<TreeNode*, double, int>>& sorted_path_scaled_frequency_space_tuples,
    int k, std::string admitted_path) {
    auto cmp = [](const std::tuple<TreeNode*, double, int>& a,
                  const std::tuple<TreeNode*, double, int>& b) {
        return std::get<1>(a) < std::get<1>(b);
    };

    std::priority_queue<std::tuple<TreeNode*, double, int>, std::vector<std::tuple<TreeNode*, double, int>>, decltype(cmp)> maxHeap(cmp);
    ...
    for (const auto& elem : sorted_path_scaled_frequency_space_tuples) {
        ...
        } else if (std::get<1>(elem) < std::get<1>(maxHeap.top())) {
            maxHeap.pop();
            maxHeap.push(elem);
            heap_size += std::get<2>(elem);
        }
    ...
    while (!maxHeap.empty()) {
        result.push_back(std::make_tuple(std::get<0>(maxHeap.top())->name, std::get<1>(maxHeap.top()), std::get<2>(maxHeap.top())));
        maxHeap.pop();
    }
