...
class TableConfigure(BfRuntimeTest):
    def setUp(self):
        ...
        bfrt_info = self.interface.bfrt_info_get("recirthpt")
        ...
        self.access_lock_key2_tbl = bfrt_info.table_get("netcacheIngress.access_lock_key2_tbl")
        ...
        self.is_recir_test_tbl = bfrt_info.table_get("netcacheIngress.is_recir_test_tbl")
    ...
    def configure_is_recir_test_tbl(self):
        keys = []
        datas = []
        for real_keydepth in range(4, 41):
            keys.append(self.is_recir_test_tbl.make_key([
                gc.KeyTuple('hdr.op_hdr.optype', GETREQ),
                gc.KeyTuple('hdr.op_hdr.real_keydepth', real_keydepth)]))
            datas.append(self.is_recir_test_tbl.make_data(
                [], 'netcacheIngress.set_is_recir_for_test'))
        self.is_recir_test_tbl.entry_add(self.target, keys, datas)
    ...
    def configure_ig_port_forward_tbl(self):
        ...
        for is_acquire_lock in is_acquire_lock_set:
            keys.append(self.ig_port_forward_tbl.make_key([
                ...
                gc.KeyTuple('meta.is_recir_for_test', 1),
            ]))
            datas.append(self.ig_port_forward_tbl.make_data(
                [gc.DataTuple('recir_port', nolock_recir_port)],
                'netcacheIngress.recir_for_recir_test'))
        
        for is_acquire_lock in is_acquire_lock_set:
            keys.append(self.ig_port_forward_tbl.make_key(
                    [gc.KeyTuple('hdr.op_hdr.optype', GETREQ),
                    gc.KeyTuple('meta.is_acquire_lock', is_acquire_lock),
                    gc.KeyTuple('meta.is_recir_for_test', 0),
                    ]))
            datas.append(self.ig_port_forward_tbl.make_data(
        ...
        for lock_reg in range(2, lock_reg_num+2):
            ...
            for tmpoptype in [PUTREQ]:
                ...
                if lock_reg == 9: 
                    ...
                    #     [gc.KeyTuple('hdr.op_hdr.optype', tmpoptype),
                    #     gc.KeyTuple('meta.is_cached_for_lock', 1),
                    #     gc.KeyTuple('hdr.op_hdr.keydepth', 2),
                    #     gc.KeyTuple('hdr.op_hdr.real_keydepth', 10),
                    #     # gc.KeyTuple('meta.is_wrong_pipeline', 0)
                    #     ]))
                    # datas.append(access_lock_key_tbl.make_data([],'netcacheIngress.acquire_key{}_lock'.format(lock_reg)))
    ...
    def configure_get_key_for_unlock_tbl(self):
        ...
        datas = []
        for keydepth in range(1, 11):
            for real_keydepth in range(keydepth + 1, 11):
                delta = real_keydepth - keydepth
                if 2 <= delta <= 9:
                    ...
                    datas.append(self.get_key_for_unlock_tbl.make_data([], 'netcacheIngress.is_key{}_unlock'.format(delta)))
        for real_keydepth in range(2, 11): # for acquire lock
            if real_keydepth < 10:
                keys.append(self.get_key_for_unlock_tbl.make_key([
                            gc.KeyTuple('hdr.op_hdr.optype', PUTREQ),
                            gc.KeyTuple('hdr.op_hdr.keydepth', 1),
            ...
            if real_keydepth == 10:
                ...
                datas.append(self.get_key_for_unlock_tbl.make_data([], 'netcacheIngress.is_key9_unlock_res'))
                keys.append(self.get_key_for_unlock_tbl.make_key([
                            gc.KeyTuple('hdr.op_hdr.optype', PUTREQ),
                            gc.KeyTuple('hdr.op_hdr.keydepth', 2),
                            gc.KeyTuple('hdr.op_hdr.real_keydepth', 10)
    ...
    def runTest(self):
        ...
        # Original entry: GETREQ + is_cached_for_lock=0 + real_keydepth in [1,10] --> reset_is_cached
        for real_keydepth in range(1, 4):
        ...
        # New entries: GETREQ + real_keydepth in [11,40] --> reset_is_cached
        # Covers extra recirculation rounds where cache hit must not be triggered
        for real_keydepth in range(4, 41):
            for is_cached_for_lock in [0, 1]:
                key = self.set_is_cached_tbl.make_key([
                    gc.KeyTuple('hdr.op_hdr.optype', GETREQ),
                    gc.KeyTuple('meta.is_cached_for_lock', is_cached_for_lock),
                    gc.KeyTuple('hdr.op_hdr.real_keydepth', real_keydepth)])
                data = self.set_is_cached_tbl.make_data([], 'netcacheIngress.reset_is_cached')
                self.set_is_cached_tbl.entry_add(self.target, [key], [data])
        ...
        print("Configuring is_recir_test_tbl")
        self.configure_is_recir_test_tbl()
