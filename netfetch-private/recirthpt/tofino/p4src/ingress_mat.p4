...
control netcacheIngress(  
	...
	action set_is_recir_for_test() {
		meta.is_recir_for_test = 1;
	}
	action reset_is_recir_for_test() {
		meta.is_recir_for_test = 0;
	}
	@pragma stage 10
	table is_recir_test_tbl {
		key = {
			hdr.op_hdr.optype: exact;
			hdr.op_hdr.real_keydepth: exact;
		}
		actions = {
			set_is_recir_for_test;
			reset_is_recir_for_test;
		}
		default_action = reset_is_recir_for_test();
		size = 64;
	}
	...
	action recir_for_recir_test(bit<9> recir_port) {
		ig_tm_md.ucast_egress_port = recir_port;
		ig_tm_md.bypass_egress = 1;
		hdr.op_hdr.real_keydepth = hdr.op_hdr.real_keydepth - 1;
	}
	@pragma stage 11
